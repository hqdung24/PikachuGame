...
{
	// Kiểm tra nếu p1.Y lớn p2.Y
	if (p1.Y > p2.Y)
	{
		// Kiểm tra nếu p1.X lớn p2.X
		if (p1.X > p2.X)
		{
			// Kiểm tra nếu đường thẳng cột p1.X trống và đường thẳng hàng p2.Y trống
			if (checkX(a, p2.Y - 1, p1.Y, p1.X, p1.X) && checkY(a, p2.X, p1.X + 1, p2.Y, p2.Y))
				return 2;
			// Kiểm tra nếu đường thẳng cột p2.X trống và đường thẳng hàng p1.Y trống
			if (checkX(a, p2.Y, p1.Y + 1, p2.X, p2.X) && checkY(a, p2.X - 1, p1.X, p1.Y, p1.Y))
				return 2;
		}
		// Kiểm tra nếu p1.X nhỏ hơn p2.X
		if (p1.X < p2.X)
		{
			// Kiểm tra nếu đường thẳng cột p1.X trống và đường thẳng hàng p2.Y trống
	...
	// Kiểm tra nếu p1.Y nhỏ hơn p2.Y
	if (p1.Y < p2.Y)
	{
		// Kiểm tra nếu p1.X lớn hơn p2.X
		if (p1.X > p2.X)
		{
			// Kiểm tra nếu đường thẳng cột p1.X trống và đường thẳng hàng p2.Y trống
	...
	// Trả về 0 nếu không có điều kiện nào thỏa
	return 0;
}
...
//Các hàm checkDraw thuật toán chạy giống như các hàm check bình thường với khác biệt là nó sẽ vẽ ra đường đi hợp lệ (nếu có)

// x đi dọc (chỉ số cột), y đi ngang(chỉ số hàng)
...
bool helpFunc(board a, int boardSize, COORD& p1, COORD& p2)
{
				...
				for (int n = 1; n < boardSize - 1; n++)
				{
					for (int m = 1; m < boardSize - 1; m++)
					{
						if ((a[i][k].KeyinBox() == a[n][m].KeyinBox()) && (n != i || m != k))
						{
							...
							p2.Y = n;
							p2.X = m;
							//kiểm tra có đường đi hợp lệ giữa 2 ô đó không. Nếu không hợp lệ thì tiếp tục tìm kiếm 2 ô giống nhau và kiểm tra.
	...
	cout << endl;
	cout << "No more steps";
	//return 0 khi không có đường đi hợp lệ
	return 0;
}

//shuffle này shuffle ngẫu nhiên hoàn toàn 
void shuffle(board& a, int boardSize)
{
	...
	do
	{
		...
		//lặp lại hàm shuffle cho đến khi có ít nhất 1 đường đi hợp lệ
	} while (!helpFunc(a, boardSize, p1, p2));
}

//shuffle2 dùng để shuffle mà vẫn giữ lại vị trí những ô chưa bị xóa
